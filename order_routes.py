        ...
        for order in orders:
            if order.product:
                ...
                product = session.query(Product).filter(
                    Product.id == order.product_id).first() if order.product_id else None
            ...
            custom_data.append({
                ...
                "total_price": order.quantity * (product.price if product else 0)
            })
...
async def update_order(id: int, order: OrderModel, Authorize: AuthJWT=Depends()):
    ...
    if order_to_update.user != current_user:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                            detail="Kechirasiz, siz boshqa foydalanuvchilarni buyurtmalarini tahrirlayolmaysiz!")
